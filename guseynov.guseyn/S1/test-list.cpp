...
{
  template< typename T >
  std::ostream & printList(std::ostream & out, guseynov::List< T > list)
  {
...
{
  guseynov::List< int > list1;
  BOOST_TEST(list1.empty());

  guseynov::List< int > list2(3, 5);
  std::ostringstream out1;
  printList(out1, list2);
  BOOST_TEST(out1.str() == "555");

  guseynov::List< int > list3({1, 2, 3});
  std::ostringstream out2;
  printList(out2, list3);
  BOOST_TEST(out2.str() == "123");

  guseynov::List< int > list4(list3.cbegin(), list3.cend());
  std::ostringstream out3;
  printList(out3, list4);
  BOOST_TEST(out3.str() == "123");

  guseynov::List< int > list5(list2);
...
{
  guseynov::List< int > list1({1, 2, 3});
  guseynov::List< int > list2({10, 20, 30});
  list1.swap(list2);
  std::ostringstream out1;
...
{
  guseynov::List< int > list({1, 2, 3, 2, 4});
  list.remove(2);
  std::ostringstream out1;
  printList(out1, list);
...
{
  guseynov::List< int > list1({1, 2, 3, 4});
  guseynov::List< int > list2({10, 20, 30});
...
BOOST_AUTO_TEST_CASE(assign)
{
  ...
  printList(out1, list);
  BOOST_TEST(out1.str() == "555");
  guseynov::List< int > list2({1, 2, 3, 4, 5, 6});
...
BOOST_AUTO_TEST_CASE(insert)
{
  ...
  it = list.cbegin();
  std::advance(it, 3);
  guseynov::ConstIterator< int > it2 = list2.cbegin();
