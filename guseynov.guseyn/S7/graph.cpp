...
guseynov::Graph::Graph(const std::string & name):
  name_(name)
{}

const std::string & guseynov::Graph::getName() const noexcept
{
  return name_;
}

bool guseynov::Graph::hasVertex(const std::string & vertex) const
{
  return edges_.find(vertex) != edges_.cend();
}

bool guseynov::Graph::addVertex(const std::string & vertex)
{
...
bool guseynov::Graph::addEdge(const std::string & from, const std::string & to, size_t weight)
{
  addVertex(from);
  addVertex(to);
...
bool guseynov::Graph::removeEdge(const std::string & from, const std::string & to, size_t weight)
{
  auto fromIt = edges_.find(from);
  if (fromIt == edges_.end())
  {
  ...
  auto & toTable = const_cast< guseynov::HashTable< std::string, std::vector< size_t > > & >(fromIt->second);
  auto toIt = toTable.find(to);
  if (toIt == toTable.end())
  {
    return false;
...
std::vector< std::string > guseynov::Graph::getVertices() const
{
...
std::vector< std::pair< std::string, size_t > > guseynov::Graph::getOutboundEdges(const std::string & vertex) const
{
...
std::vector< std::pair< std::string, size_t > > guseynov::Graph::getInboundEdges(const std::string & vertex) const
{
  std::vector< std::pair< std::string, size_t > > result;
