...
namespace averenkov
{
  ...
  private:
    const averenkovDetail::Node< Key, Value >* current;
    explicit ConstIterator(const averenkovDetail::Node< Key, Value >* node);
  ...
  template < class Key, class Value, class Compare >
  ConstIterator< Key, Value, Compare >::ConstIterator(const averenkovDetail::Node< Key, Value >* node):
    current(node)
  {
  }
